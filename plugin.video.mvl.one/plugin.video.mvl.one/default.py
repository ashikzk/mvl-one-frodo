...
def dialog_msg():
    ...
    heading = "SYSTEM BUSY"
    text = "Oops! You were unable to connect to MVL. Please try again."

    #show message is a dialog window
    showMessage(heading, text)


def internet_dialog_msg():
    ...
    try:
        while(True):
            ...
            if path == '':
                ...
                if last_path and 'show_popup' not in last_path:
                    show_home = True
            elif 'plugin://plugin.video.mvl' not in path:
                show_home = True
            else:
                show_home = False

            if show_home:
                # showMessage('Wrong Place', 'Going Home')
